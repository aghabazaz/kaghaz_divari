...
//    private $product_product_tbl          = 'shop_product_gift' ;
    ...
    public function orderItemSave ()
    {
        ...
        $paramsDynamic['grayscale']=$params['grayscale'];
        $paramsDynamic['mirror']=$params['mirror'];
        $paramsDynamic['size']=$params['size'];
        $paramsDynamic['position_cut']=$params['positionCut'];
        $paramsDynamic['pic_name']=$params['pic_name'];
        $paramsDynamic['product_pic']=$params['product_pic'];
        $paramsDynamic['shop_product_price_id']=$params['product_price_id'];
        $product_type=$params['typeProduct'];
        unset($params['grayscale']);
        unset($params['mirror']);
        unset($params['size']);
        unset($params['positionCut']);
        unset($params['pic_name']);
        unset($params['typeProduct']);
        if($product_type=='dynamic'){
            $params['dynamic']='on';
            $this->sqlEngine->Select ()
                ->From (  'shop_order_item_dynamic AS t1' )
                ->innerJoin('shop_order_items AS t2')
                ->on('t2.shop_order_items_id=t1.id')
                ->innerJoin('shop_orders AS t3')
                ->on('t2.order_id=t3.id')
                ->Where ( 't1.grayscale=?', $paramsDynamic[ 'grayscale' ] )
                ->andWhere ( 't1.mirror=?', $paramsDynamic[ 'mirror' ] )
                ->andWhere ( 't1.position_cut=?', $paramsDynamic[ 'position_cut' ] )
                ->andWhere ( 't1.pic_name=?', $paramsDynamic[ 'pic_name' ] )
                ->andWhere ( 't1.shop_product_price_id=?', $paramsDynamic[ 'shop_product_price_id' ] )
                ->andWhere('t1.size=?', $paramsDynamic[ 'size' ])
                ->andWhere('t3.status=?','cart')
                ->Run();
            $row=$this->sqlEngine->getRows () ;
            if(count($row)>0)
            {
                unset($params['order_id']);
                unset($params['product_price_id']);
                unset($params['date_register']);
                unset($params['discount_type']);
                unset($params['discount_price']);
                unset($params['price']);
                unset($params['status']);
                unset($params['type_discount']);
                unset($params['dynamic']);
                $this->sqlEngine->Select ()
                    ->From (  'shop_order_items AS t1' )
                    ->Where ( 't1.id=?',$row[0]['shop_order_items_id']  )
                    ->Run () ;
                $row1=$this->sqlEngine->getRow () ;
                $count=$row1['count']+1;
                $params['count']=$count;
                $result = $this->sqlEngine->save($this->orderItem_tbl, $params,array(
                    'id=?',array($row[0]['shop_order_items_id'])));
            }else{
                $result = $this->sqlEngine->save($this->orderItem_tbl, $params);
                $paramsDynamic['shop_order_items_id']=$result;
                $paramsDynamic['shop_product_price_id']=$params['product_price_id'];
                $result2 = $this->sqlEngine->save('shop_order_item_dynamic', $paramsDynamic);
            }
        }else {
            unset($params['product_pic']);
            $result = $this->sqlEngine->save($this->orderItem_tbl, $params);
        }
        return $result ;
    }
    ...
    public function getOrderItemByParams ()
    {
        ...
        //\f\pre($param);
        $this->sqlEngine->Select ( 't7.id AS order_id,shop_material.name AS materialName,t1.dynamic,shop_order_item_dynamic.size,shop_order_item_dynamic.product_pic,t1.id AS orderItem_id,t1.id,t1.product_price_id,t1.count,t1.date_register,
 ...
 AS discountEnd,t1.price,
  ...
  t1.type_discount,
t1.discount_price,t1.gift,t2.stock,t3.id product_id,t3.title AS productTitle,t3.sub_title AS productTitleSub,t3.picture,t4.id AS color_id,t4.title AS colorTitle,t4.code AS colorCode,t5.title AS guranteeTitle,t3.m_free,t3.n_buy,t3.active_gift_section,t3.shop_product_gift_id' )
            ->From ( $this->orderItem_tbl . ' AS t1' )
            ...
            ->leftJoin('shop_order_item_dynamic')
            ->On('shop_order_item_dynamic.shop_order_items_id=t1.id')
            ->leftJoin('shop_material')
            ->On('shop_material.id=t2.material_id')
            ->Where ( 1 ) ;
        if ( $param[ 'order_id' ] )
        {
    ...
    (t9.price IS Null OR t9.price<=0),
				IF(STRCMP(t2.type_discount,"percent")=0,(t1.price*(t2.discount/100)),t2.discount),
    			IF(STRCMP(t9.discount_type,"percent")=0,(t1.price*(t9.price/100)),t9.price)
    ) AS discountEnd,
        IF(
    (t9.price IS Null OR t9.price<=0),
				t2.discount,
    ...
    "default","amazing") AS discount_type,
          ...
          //  \f\pr($param);
          //  \f\pre($this->sqlEngine->last_query());
        if(!empty($row) && $param['productType']!='dynamic') {
            foreach ($row as $data) {
                $this->sqlEngine->save($this->orderItem_tbl,
                    array(
                        // 'price' => $data['price'],
   ...
   public function getOrderItemByParamsCart ()
    ...
    (t9.price IS Null OR t9.price<=0),
				IF(STRCMP(t2.type_discount,"percent")=0,(t1.price*(t2.discount/100)),t2.discount),
    			IF(STRCMP(t9.discount_type,"percent")=0,(t1.price*(t9.price/100)),t9.price)
    ) AS discountEnd,IF(t1.product_price_id is Null,(t1.price),
     IF(
    (t9.price IS Null OR t9.price<=0),
				IF(STRCMP(t2.type_discount,"percent")=0,  
                 t2.price-(t1.price*(t2.discount/100)),t1.price-t2.discount),
    			IF(STRCMP(t9.discount_type,"percent")=0,t1.price-(t1.price*(t9.price/100)),t1.price-t9.price)
    )) AS priceEnd,
        IF(
    (t9.price IS Null OR t9.price<=0),
				t2.discount,
    ...
    (t9.price IS Null OR t9.price<=0),
     ...
     IF(
    (t9.price IS Null OR t9.price<=0),
    "default","amazing") AS discount_type,t1.product_pic,
          IF(t1.product_price_id is Null,t1.price,t1.price) AS price,t1.dynamic AS dynamicOrderItem,
            t2.user_price,t11.size,shop_category.dynamic,t10.type_user,t1.gift,t2.stock,t3.id product_id,t3.title AS productTitle,t3.sub_title AS productTitleSub,t3.picture,t4.id AS color_id,t4.title AS colorTitle,t4.code AS colorCode,t5.title AS guranteeTitle,t3.m_free,t3.n_buy,t3.active_gift_section,t3.shop_product_gift_id,t1.description,t1.custom_request,t1.size AS custom_request_size' )
            ->From ( $this->orderItem_tbl . ' AS t1' )
            ->leftJoin ( $this->product_price_tbl . ' AS t2' )
            ->On ( 't1.product_price_id=t2.id' )
            ->leftJoin ( 'shop_product AS t3' )
            ...
            ->On('shop_category.id=t3.shop_category_id')
            ->leftJoin('shop_order_item_dynamic AS t11')
            ->On('t11.shop_order_items_id=t1.id')
            ->Where ( 1 ) ;
        if ( $param[ 'order_id' ] )
        {
            $this->sqlEngine->andWhere ( 't7.id=?', $param[ 'order_id' ] ) ;
        ...
        if ( $param[ 'status' ] ){
            //get just orderItems which into cart from frontend
            $this->sqlEngine->andWhere ( '(t3.status="enabled" or t3.status is null)' ) ;
            $this->sqlEngine->andWhere ( 't7.status=?', $param[ 'status' ] ) ;
        }
        if($param['gift']=='no'){
            $this->sqlEngine->andWhere ( 't1.gift=?', 'no' ) ;
    ...
    public function getOrderItemByParamsReview ()
    {
        ...
        t1.type_discount,shop_order_item_dynamic.size,t1.id AS orderItem_id,t1.id,t1.product_price_id,t1.count,t1.date_register,t1.product_pic,shop_category.dynamic,
    	IF(STRCMP(t1.type_discount,"percent")=0,(t1.price*(t1.discount_price/100)),t1.discount_price)  AS discountEnd,
          t1.price,
    ...
    public function checkOrderItemsOldPriceId ()
    {
     ...
     //\f\pr($param);
        $this->sqlEngine->Select ( 't1.id,t1.count' )
                ...
                if($param['typeProduct']=='dynamic') {
                    $this->sqlEngine->andWhere('t1.id=?', $param['order_item_id']);
                }
        $this->sqlEngine->Run () ;
             //   \f\pr('slam');
           //    \f\pr($param);
    ...
    public function checkRepeatOrderItemDynamicProduct(){
        $param = $this->request->getAssocParams () ;
        //\f\pr($param);
        $this->sqlEngine->Select ( 't2.id AS order_item_id,t2.count' )
            ->From ( 'shop_order_item_dynamic AS t1' )
            ->innerJoin('shop_order_items  AS t2')
            ->On ( 't2.id = t1.shop_order_items_id' )
            ->innerJoin('shop_orders AS t3')
            ->On('t3.id=t2.order_id')
            ->Where ( "t1.grayscale=?",$param['grayscale'])
            ->andWhere ( "t1.mirror=?",$param['mirror'])
            ->andWhere("t1.pic_name=?",$param['pic_name'])
            ->andWhere("t1.position_cut like '%".trim($param['position_cut'])."%'")
            ->andWhere("t1.shop_product_price_id=?",$param['product_price_id'])
            ->andWhere("t3.user_id=?",$param['user_id'])
            ->andWhere("t3.status=?",'cart')
            ->andWhere ( "t1.size like '%". $param[ 'size' ]."%'" )
            ->Run () ;
     // \f\pr($param);
      //   \f\pr($this->sqlEngine->last_query());
       //\f\pre($this->sqlEngine->getRow());
