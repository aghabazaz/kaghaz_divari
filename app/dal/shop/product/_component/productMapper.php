...
class productMapper extends \f\dal
{
    ...
    public function productSave()
    {
        ...
        //\f\pre(str_replace('"','',str_replace(']','',str_replace('[','',json_encode($params['category'])))));
            ...
            if ($params['idPrice'][$i]) {
                $this->sqlEngine->save($this->product_price_tbl,
                    array(
                        ...
                        'user_price' => ($user_price==''?NULL:$user_price),
                        'discount' => ($discount==''?NULL:$discount),
                        'type_discount' => ($params['type_discount'][$i]?NULL:$discount),
                        'dynamic'=>($params['dynamic']==''?true:$params['dynamic'])
                    ),
                    array(
                        'id=?',
                        array(
...
//\f\pre($this->sqlEngine->last_query());
    ...
    public function getPriceByProductById()
    {
        ...
        $dynamic = $this->request->getParam('dynamic');
        ...
        $this->sqlEngine->Select()
            ...
            ->Where('shop_product_id=?', $id);
        if($dynamic=='dynamic'){
            $this->sqlEngine->OrderBy('majorPrice ASC');
        }else{
            $this->sqlEngine->OrderBy('price  ASC');
        }
        if ($history) {
            $this->sqlEngine->OrderBy('id ASC');
        }
        $this->sqlEngine->Run();
    ...
    public function getProductByParams()
    {
        ...
        $this->sqlEngine->Select('t1.id,t1.shop_product_id,t1.stock')
            ...
            ->andWhere("t2.shop_category_id IN(". $param['cat_id'].")")
    ...
    public function getNewOneProduct()
    {
        ...
        $this->sqlEngine->Select('t1.title,t1.shop_category_id,t1.picture,t1.id,t2.price AS shopKepperPrice,t2.user_price AS user_price,t2.majorPrice AS majorPrice,t2.stock AS stock,t2.discount,shop_category.title_en,t2.type_discount,t3.price AS amazingPrice,t3.discount_type AS discountTypeAmazing')
            ->From($this->product_tbl . ' AS t1')
            ->leftJoin($this->product_price_tbl . ' AS t2')
            ->On('t1.id=t2.shop_product_id')
            ->innerJoin('shop_category')
            ->On('shop_category.id=t1.shop_category_id')
            ->leftJoin($this->shop_amazing.' AS t3')
            ->On('t3.shop_product_id=t1.id AND t3.date_start>='.$todayDate.' AND t3.date_end<='.$todayDate)
            ->Where('t1.owner_id=?', $ownerId)
            ->andWhere('t1.shop_category_id IN (' . $param['categoryId'] . ')')
            ->andWhere('t1.status=?', 'enabled')
            ->andWhere('t2.stock>0')
            ->GroupBy('t1.id DESC')
            ->Limit(10)
 ...
 //       \f\pre($this->sqlEngine->getRow());
    ...
    public function getProductList(){
        $param = $this->request->getAssocParams();
        $this->sqlEngine->Select('t1.id,t1.title')
