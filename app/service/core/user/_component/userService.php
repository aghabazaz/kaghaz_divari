...
# Propagating the standarded event
    ...
    public function saveOldBackendUserToNewSystem()
    {
        $userId = \f\ttt::dal('core.user.saveOldBackendUserToNewSystem',
                              $this->request->getAssocParams()) ;
        return $userId ;
    }
    ...
    {
        return \f\ttt::dal('core.user.getUserByOwnerId',
                           $this->request->getAssocParams()) ;
    }
    ...
    public function saveUserPerms()
    {
        ...
        \f\ttt::dal('core.rbac.user2.delUserRoles',
                    array (
            'userId' => $params[ 'userId' ]
        )) ;

        \f\ttt::dal('core.rbac.user2.delUserPerms',
                    array (
            'userId' => $params[ 'userId' ]
        )) ;
        ...
        {
            \f\ttt::dal('core.rbac.user2.saveUserRoles',
                        array (
                'userId' => $params[ 'userId' ],
                'roles'  => $params[ 'roles' ]
            )) ;
        ...
        if ( ! empty($params[
                        'perms' ]) )
        {
            \f\ttt::dal('core.rbac.user2.saveUserPerms',
                        array (
                'userId' => $params[ 'userId' ],
                'perms'  => $params[ 'perms' ]
            )) ;
