...
// it to the browser
    ...
    //when add or mines item or cash on payment
    public function orderSave () {
        $params = $this->request->getAssocParams();

        //برای مواقعی است که پرداخت در محل باشد
        if ( $params['id'] ) {
            ...
            $params['status'] = 'cart';
            //آیا این کاربر قبلا در سبد خریدش محصولی را سفارش داده است یا خیر
            $checkOrderCart   = \f\ttt::dal( 'shop.order.checkOrderCartByUserId', $params );

            //check if not exsist cart old then insert new order cart else add to orderItem
            if ( $checkOrderCart == NULL )
            {
                //add to shop_order record
            ...
            //برای مواردی که محصول از نوع استاتیک باشد و موجودی دارد و یا اینکه محصول در دسته بندی های داینامیک باشد
            if ( $checkStock != NULL or $params['typeProduct']=='dynamic')
            {
                ...
                if($params['typeProduct']=='dynamic' && $params['order_item_id']==''){
                    ...
                    // price_id برای محصولات داینامیک بایستی سایز تصویر
                    //  تصویر سیاه وسفید و یا اینکه برعکس است یا خیر چک شود اگر قبلا این مشخصات برای همان تصویر با همان بود یعنی محصول تکراری است
                ...
                /*
                //اگر قبلا این محصول با price_id مربوطه ایجاد شده بود
                */
                if ( is_array( $result ) != NULL )
                {
                    ...
                    /*
                     برای حالتی که محصول با price_id مشخص قبلا درج نشده است
                     */
                    ...
                    $result   = \f\ttt::dal( 'shop.orderItem.orderItemSave',$params );
    ...
    public  function calculatOrderEndPrice ()
    {
        $params = $this->request->getAssocParams();
       // \f\pr($params);
        $row = \f\ttt::dal( 'shop.orderItem.getOrderItemByParamsCart',$params );
      //  \f\pre($row);
        foreach ( $row AS $data )
        {
            if ( $data['count'] != 0 )
    ...
    public  function sendIdTransAndGoToReview ()
    {
        ...
        //save transportation cost to shop_order table
        $param['transportation_cost'] = $trans_price['cost'];
        $result                       = \f\ttt::dal( 'shop.order.orderSaveEdit',$param );
        if ( $result )
        {
            $data = [
                'result'  => 'success',
...
//    }
    ...
    public function saveCustomReqOrder(){
        $params=$this->request->getAssocParams();
        $params['status'] = 'cart';
        //\f\pre($params);
        //آیا این کاربر قبلا در سبد خریدش محصولی را سفارش داده است یا خیر
        ...
        //\f\pre($_SESSION['order_id']);
        return \f\ttt::dal( 'shop.order.addCustomOrderToOrderItem',
            $params );
    }
