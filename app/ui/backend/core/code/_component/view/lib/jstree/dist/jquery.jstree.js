		...
		_fn : {
			_show_input : function (obj, callback) {
				...
				var w = this.get_settings().crrm.input_width_limit,
					t = this.get_text(obj),
					h1 = $("<div>", { css : { "position" : "absolute", "top" : "-200px", "left" : "0px", "visibility" : "hidden" } }).appendTo("body"),
					h2 = $("<input>", { 
						"value" : t,
						// "size" : t.length,
						"css" : {
					...
					}).insertAfter(obj.css("position","relative").find("> a"));
				this.set_text(obj, "");
				h1.css({
...
(function ($) {
	...
	$.jstree.plugin("dnd", {
		...
		defaults : {
			...
			dir				: false // Could be 'rtl', 'ltr', or false to autodetect
		},
		_fn : {
			dnd_prepare : function () {
				this.data.dnd.off = r.offset();
				this.data.dnd.off.right = r.offset().left + r.width();
			...
			dnd_show : function () {
				...
				var rtl = (this.get_settings().dnd.dir || $.vakata.dnd.helper.css('direction'))=='rtl',
					left = rtl?(this.data.dnd.off.right - 18):(this.data.dnd.off.left + 10);
				switch(r) {
					case "before":
						m.css({ "left" : left + "px", "top" : (this.data.dnd.off.top - 6) + "px" }).show();
						break;
					case "after":
						m.css({ "left" : left + "px", "top" : (this.data.dnd.off.top + this.data.core.li_height - 7) + "px" }).show();
						break;
					case "inside":
						m.css({ "left" : (left + (rtl?-4:4)) + "px", "top" : (this.data.dnd.off.top + this.data.core.li_height/2 - 5) + "px" }).show();
						break;
					default:
						m.hide();
						break;
	...
	$(function() {
		var css_string = '' + 
			...
			'#jstree-marker { padding:0; margin:0; line-height:12px; font-size:1px; overflow:hidden; height:12px; width:8px; position:absolute; top:-30px; z-index:1000; background-repeat:no-repeat; display:none; background-color:silver; } ';
		$.vakata.css.add_sheet({ str : css_string });
