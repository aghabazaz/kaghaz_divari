...
class productController extends \f\controller
{
    ...
    public function productIndex ()
    {
        ...
        else
        {
            ...
            //\f\pre($nonAssocParams);
            $content  = $this->view->renderGetProduct ( array_merge($nonAssocParams,[$title]) ) ;
        }
    ...
    public function productRateSave ()
    {
        ...
        $row                 = \f\ttt::service ( 'shop.ratingOptions.saveRatingOptionsScore',$params ) ;
    ...
    {
        $params  = $this->request->getAssocParams () ;
       // $bestsellingSetting = \f\ttt::service ( 'cms.setting.getSettings');
    ...
    public function sendAddToCart ()
    {
       ...
       // \f\pre('hi');
        ...
        //when add dynamic product on index page then go to details product
       /* if($params['page_add']=='indexPage'){
            $result = array (
                'result'  => 'errorOnProductDetails',
                'productId'=>$params['product_id']
            );
            return $this->response ( $result ) ;
        }*/

        if($params['typeProduct']=='dynamic'){
            ...
            if(preg_match("/\/(\d+)$/",$params['image'],$matches)){
                $picName=$matches[1];
            }
            ...
            // Resample
            //for save image with specified size,mirror,grayscale image
            ...
            $params['product_pic']=$picture;
            $params['pic_name']=$picName;
            $params['price']=$params['product_price'];
            $params['positionCut']=$transactionImage;
        } else {
        ...
        return $this->response ( array (
                    ...
                    'topItem'=>$params['topItem']
                ) ) ;
    }

    public function sendAddToCartCusPro(){
        $params = $this->request->getAssocParams () ;
        \f\pre('dsf');
        if (empty($_SESSION[ 'user_id' ]) or !isset($_SESSION['user_id']) ){
            $result = array (
    ...
    public function getSpecialCategory(){
        $content = $this->view->renderGetSpecialCategory () ;
        return $this->renderPartial ( $content ) ;
    }
    ...
    public function sendCustomProductRequest(){
        ...
        if (empty($_SESSION[ 'user_id' ]) or !isset($_SESSION['user_id']) ){
            $result = array (
                'result'  => 'error',
                'message' => 'جهت ادامه فرآیند خرید ورود کاربر الزامی است.',
                'url'     => \f\ifm::app ()->siteUrl.'login'
            ) ;
            return $this->response ( $result ) ;
        }

        if($params['width']=='' or $params['height']=='' or $params['material']=='' or $params['image']==''){
            $result = array (
                'result'  => 'error',
                'message' => 'پرکردن فیلدهای ستاره دار الزامی است.',
            );
            return $this->response ( $result ) ;
        }
        ...
        $row=\f\ttt::service('shop.order.saveCustomReqOrder',$params) ;
       // sendCustomProductRequest
        //\f\pre($row);
      //  return $this->response ( $row ) ;
    }
    public function submitCustomReq(){
        ...
        $fileName = $_FILES['files']['file']['fileImg']['name'];
        $fileSize = $_FILES['files']['file']['size'];
        $fileTmpName  = $_FILES['files']['file']['tmp_name'];
        $fileType = $_FILES['files']['file']['type'];
        ...
        $uploadPath = $currentDir."\upload\shop\customRequest"."\\" . basename($name);
        $didUpload = move_uploaded_file($fileTmpName, $uploadPath);
        $savePath=\f\ifm::app()->siteUrl."upload/shop/customRequest/".$name;
        $params['pic']=$savePath;
