...
<input type="hidden" id="itemPriceVal" />
...
<script type="text/javascript">
    ...
    //برای تبدیل اعداد فارسی و کاما دار به اعداد انگلیسی
...
<script type="text/javascript">
    getGurantee();
    //براساس رنگ مشخص شده گارانتی های مربوط به آن را نشان می دهد
    ...
    //نمایش قیمت و تخفیف براساس نوع رنگ و گارانتی انتخاب شده
        ...
        if (params.gurantee != 'NULL') {
            if (amazingPrice) {
                if(amazingDisType=='percent'){
                    ...
                    priceVal=params.gurantee['user_price'][id] *((100- amazingPrice)/100);
                ...
                if(params.gurantee['type_discount'][id]=='percent'){
                    ...
                    priceVal=params.gurantee['user_price'][id] *((100-params.gurantee['id'][id])/100);
                }else if(params.gurantee['type_discount'][id]=='fixed'){
                    price = toTomanComment(params.gurantee['user_price'][id] - params.gurantee['id'][id]);
                    priceVal=params.gurantee['user_price'][id] - params.gurantee['id'][id];
                }
            }
    ...
    //به هنگام تغییر گارانتی قیمت متناسب با گارانتی و رنگ انتخاب شده نمایش داده می شود
    ...
    /*
     هنگامی که در صفحه جزئیات محصول روی دکمه افزودن محصول کلیک می کنیم
      با استفاده از کنترلر یکی به تعداد محصولات اضافه کرده و سپس به صفحه کارت خرید می رود
    */
    function sendAddToCart() {
        ...
        var option = {
            product_price_id: $(".button-addToCart").attr("data"),
            product_id: id,
            typeOpr:'addToCart',
            typeProduct:'static',
            price:$('#itemPriceVal').val()
        };
        widgetHelper.tt('ui', 'shop.product.sendAddToCart', option, 'getBasketSidebar');
    }

    //برای رفتن به قسمت امتیازدهی به کالا
    function goToRate() {
    ...
    //عملیات مورد نظر برای اضافه نمودن محصول به کالای مورد علاقه
    ...
    //برای اضافه کردن به علاقه مندی ها
