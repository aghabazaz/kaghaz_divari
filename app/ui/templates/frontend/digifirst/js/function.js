(function ($) {
	...
	function init_carousel() {
		...
		$(".owl-carousel").each(function (index, el) {
			...
			config.navText = ['<i class="fa fa-angle-right" aria-hidden="true"></i>', '<i class="fa fa-angle-left" aria-hidden="true"></i>'];
			config.rtl = true;
	...
	function tab_product_fade_effect() {
		...
		$(document).on('click', '.tab-product-fade-effect a[data-toggle="tab"]', function () {
			...
			var i = -1;
			$(tab_id).find('.product-item').each(function () {
				if ($(this).parent().find('~ .item-first').length == 0) {
					i++;
				}
				var t = $(this);
	...
	$(document).ready(function () {
		...
		$(document).on('click', '.colse-cate', function () {
				...
				$(this).addClass('open-cate').removeClass('colse-cate').html('+ همه دسته ها');
				return false;
			})
		...
		$('.slider-range-price').each(function () {
			...
			$(this).slider({
				...
				slide: function (event, ui) {
					var result = label_reasult + '<span>' + ui.values[1] + ' ' + unit + '</span>-<span>' + ui.values[0] + ' ' + unit + '</span>';
