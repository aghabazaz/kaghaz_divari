...
class slideView extends \f\view
{
    ...
    public function renderSlideAdd ( $id = '' )
    {
       
        if ( $id )
        {
            $row = \f\ttt::service ( 'cms.slide.getSlideById',
                                     array (
                        'id' => $id ) ) ;
        }
    ...
    public function renderSlideGrid ( $requestDataTble )
    {
        ...
        $slideList = \f\ttt::service ( 'cms.slide.slideList',
                                       array (
                    'dataTableParams' => $requestDataTble ) ) ;

        foreach ( $slideList[ 'data' ] as $key => $value )
        {

            $tdContent = $this->createActionButtons ( $value, 'slide' ) ;
        ...
        $row[ 'total' ] = $slideList[ 'total' ] ;
        $row[ 'draw' ]  = $slideList[ 'draw' ] ;
        /* @var $table \f\w\table */
...
//            ),
            array (
                'type' => 'edit',
                'href' => \f\ifm::app ()->baseUrl . "cms/slide/" . $section . "Edit/" . $data[ 'id' ]
            ),
            array (
                'type'           => 'status',
                'confirm'        => TRUE,
                'id'             => 's' . $data[ 'id' ],
                'status'         => $data[ 'status' ],
                'action'         => 'cms.slide.' . $section . 'Status',
            ...
            array (
                'type'           => 'delete',
                'confirm'        => true,
                'action'         => 'cms.slide.' . $section . 'Delete',
