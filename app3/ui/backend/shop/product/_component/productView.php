...
class productView extends \f\view
{
    ...
    public function renderProductAdd ( $id = '' )
    {
        if ( $id )
        {
            ...
            $oldCategoryAll=$row['shop_category_id'];

           $arr1=explode(',',$row['shop_category_id']);
           //\f\pre($arr1);
           $oldCat=$arr1[0];
            $row['shop_category_id']=json_encode($arr1);

        // \f\pre($row['shop_category_id']);
            $price = \f\ttt::service ( 'shop.product.getPriceByProductById',array ('id' => $id ) ) ;
       ...
       // \f\pre($sort);
        ...
        foreach ($sort AS $item){
            ...
            $arrayCat[$item['id']]=$item['title'];
            $arrayCatD[$item['id']]=$item['dynamic'];
        }
        //\f\pre($arrayCatD);
...
//\f\pr($params);
        $id=$params['id'][0];
        $oldCat=(int)$params['oldCat'];
        if($id!=null or $id!=''){
            if($oldCat>0 and array_key_exists('lastCat',$params)){
                $row1 = \f\ttt::service ( 'shop.product.getFeatureByCatId',
                    array (
                        'id' => $params['oldCat']
                    ) ) ;
             ...
             //  \f\pr($params['id'][0]);
             // \f\pr($row1);
                $row2 = \f\ttt::service ( 'shop.product.getFeatureByCatId',
                    array (
                        'id' => $params['lastCat']
                    ) ) ;
               // \f\pr($params['lastCat']);
            //    \f\pr($row2);
                $result=array_diff($row1,$row2);
