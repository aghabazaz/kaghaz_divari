...
//                'content' => $params['content'],
    ...
    public function productSaveEdit()
    {
       ...
       // \f\pre($params);
        //\f\pre($params);
    //    \f\pre($params['short_product_id'][0]);
...
//\f\pre($this->sqlEngine->last_query());
    ...
    public function getNewProduct()
    {
        ...
        $this->sqlEngine->Select('t2.id AS product_price_id,t1.title,t1.picture,t1.id,t2.price,t2.stock,t2.discount,type_discount')
            ->From($this->product_tbl . ' AS t1')
...
if(t2.discount=0 or t2.discount IS NULL, 
 ...
 if(t2.discount=0 or t2.discount IS NULL, 
    ...
    public function getMostVisitProduct(){
        $param = $this->request->getAssocParams();
        $this->sqlEngine->Select('t1.*,shop_product_price.*')
            ->From($this->product_tbl . ' AS t1')
            ->innerJoin('shop_product_price')
            ->On('shop_product_price.shop_product_id=t1.id')
            ->Where('t1.status=?', 'enabled')
