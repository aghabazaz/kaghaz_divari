...
class returnedView extends \f\view
{
    ...
    public function renderReturnedDetail ( $id = '' )
    {
        if ( $id )
        {
    ...
    public function renderReturnedGrid ( $requestDataTble )
    {
        /** Get group list * */
        $this->registerGadgets ( array (
            'dateG' => 'date' ) ) ;
        $returnedList = \f\ttt::service ( 'shop.order.orderListReturned',
                                          array (
                    'dataTableParams' => $requestDataTble ) ) ;

        foreach ( $returnedList[ 'data' ] as $key => $value )
        {
            $tdContent = $this->createActionButtons ( $value, 'returned' ) ;
...
//                ),
        ...
        $row[ 'total' ] = $returnedList[ 'total' ] ;
        $row[ 'draw' ]  = $returnedList[ 'draw' ] ;
        /* @var $table \f\w\table */
    ...
    private function createActionButtons ( $data, $section )
    {
        $buttonsParam = array (
            array (
                'type' => 'details',
                'href' => \f\ifm::app ()->baseUrl . "accountingStock/returned/returnedDetail/" . $data[ 'id' ]
            ),
            array (
                'type'           => 'delete',
                'confirm'        => true,
                'action'         => 'accountingStock.returned.' . $section . 'Delete',
