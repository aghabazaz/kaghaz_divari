...
$form.='<div class="col-sm-10">'
        ...
        . '<div class="col-sm-9">'
        . '<select name="category" id="category" onChange="loadFeature()" data-parsley-required=""><option></option>' ;
...
<div style="border:1px solid #ddd " id="paramBox">
    ...
    <div class="bodyParam">
      ...
      //  \f\pre($_SESSION);
       ...
       // \f\pr($_SESSION);
        if($_SESSION['typeCat']==true){
            $dynamic='true';
        }else{
            $dynamic='false';
        }

        //\f\pre(!empty($majorPrices) and $dynamic='true');
        //\f\pr($dynamic);
        //\f\pre($_SESSION);
...
console.log(row);
    ...
    function loadFeature()
    {
        ...
        if (oldCat)
        {
            $('#catDialog').modal({backdrop: 'static', keyboard: false});
        } else
        {
            loadData();
        }
    ...
    function loadData()
    {
        if($('#category').children(":selected").attr("data-dynamic")=='true'){
        ...
        var params = {
            id: $('#category').children(":selected").attr("id"),
            pId: $('#id').val(),
            catType:$('#typeCat').val()
        };

        widgetHelper.tt('ui', 'shop.product.loadFeature', params, 'featureForm');
        $('#oldCat').val($('#category').val());
    }
    function cancelLoadData()
    {
        $('#category').select2('val', $('#oldCat').val());
        $('#category').val($('#oldCat').val());
    }
