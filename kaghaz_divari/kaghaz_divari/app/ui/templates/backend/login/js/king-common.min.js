$(document).ready(function () {
    $li = $('.main-menu li.active').find("ul").addClass('open');
    ...
    $(".js-toggle-minified").clickToggle(function () {
        ...
        $(".sidebar-minified").find("i.fa-angle-right").toggleClass("fa-angle-left").toggleClass("fa-angle-right")
    }, 
    function () {
        $(".left-sidebar").removeClass("minified"), 
        $(".content-wrapper").removeClass("expanded"), 
        $(".sidebar-minified").find("i.fa-angle-left").toggleClass("fa-angle-left").toggleClass("fa-angle-right")
    }), 
    ...
    $("body").tooltip({selector: "[data-toggle=tooltip]", container: "body"}), $(".alert .close").click(function (e) {
        e.preventDefault(), 
        $(this).parents(".alert").fadeOut(300)
    }), 
    $(".btn-help").popover({container: "body", placement: "top", html: !0, title: '<i class="fa fa-book"></i> Help', content: "Help summary goes here. Options can be passed via data attributes <code>data-</code> or JavaScript. Please check <a href='http://getbootstrap.com/javascript/#popovers'>Bootstrap Doc</a>"}), 
    $(".demo-popover1 #popover-title").popover({html: !0, title: '<i class="fa fa-cogs"></i> Popover Title', content: "This popover has title and support HTML content. Quickly implement process-centric networks rather than compelling potentialities. Objectively reinvent competitive technologies after high standards in process improvements. Phosfluorescently cultivate 24/365."}), 
    $(".demo-popover1 #popover-hover").popover({html: !0, title: '<i class="fa fa-cogs"></i> Popover Title', trigger: "hover", content: "Activate the popover on hover. Objectively enable optimal opportunities without market positioning expertise. Assertively optimize multidisciplinary benefits rather than holistic experiences. Credibly underwhelm real-time paradigms with."}), 
    $(".demo-popover2 .btn").popover(), 
    $(".widget-header-toolbar .btn-ajax").click(function (e) {
        e.preventDefault(), 
        $theButton = $(this), 
        $.ajax({url: "php/widget-ajax.php", type: "POST", dataType: "json", cache: !1, beforeSend: function () {
    ...
    $(".widget-header .multiselect").length > 0 && $(".widget-header .multiselect").multiselect({dropRight: !0, buttonClass: "btn btn-warning btn-sm"}), 
    ...
    $(".bs-switch").length > 0 && $(".bs-switch").bootstrapSwitch(), 
    $(".demo-only-page-blank").length > 0 && $(".content-wrapper").css("min-height", $(".wrapper").outerHeight(!0) - $(".top-bar").outerHeight(!0));
